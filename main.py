...
def chooseScreen():
    ...
    DISPLAYSURF.fill(BGCOLOR)
    chooseText = BASICFONT.render('Choose the grid size...', True, WHITE)
    chooseTextRect = chooseText.get_rect()
    chooseTextRect.center = (WINDOWWIDTH / 2, WINDOWHEIGHT / 4)

    three = BASICFONT.render('3 X 3', True, WHITE)
    threeRect = three.get_rect()
    threeRect.center = (WINDOWWIDTH / 4, WINDOWHEIGHT / 2)

    four = BASICFONT.render('4 X 4', True, WHITE)
    fourRect = four.get_rect()
    fourRect.center = (3 * WINDOWWIDTH / 4, WINDOWHEIGHT / 2)

    DISPLAYSURF.blit(chooseText, chooseTextRect)
    DISPLAYSURF.blit(three, threeRect)
    DISPLAYSURF.blit(four, fourRect)
    pygame.display.update()
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if threeRect.collidepoint(mouseX, mouseY):
                    return 3
                elif fourRect.collidepoint(mouseX, mouseY):
                    return 4
...
def difficultyScreen():
    DISPLAYSURF.fill(BGCOLOR)
    chooseText = BASICFONT.render('Choose difficulty...', True, WHITE)
    chooseTextRect = chooseText.get_rect()
    chooseTextRect.center = (WINDOWWIDTH / 2, WINDOWHEIGHT / 4)

    easy = BASICFONT.render('EASY', True, WHITE)
    easyRect = easy.get_rect()
    easyRect.center = (WINDOWWIDTH / 6, WINDOWHEIGHT / 2)

    med = BASICFONT.render('MEDIUM', True, WHITE)
    medRect = med.get_rect()
    medRect.center = (WINDOWWIDTH / 2, WINDOWHEIGHT / 2)

    hard = BASICFONT.render('HARD', True, WHITE)
    hardRect = hard.get_rect()
    hardRect.center = (5 * WINDOWWIDTH / 6, WINDOWHEIGHT / 2)

    DISPLAYSURF.blit(chooseText, chooseTextRect)
    DISPLAYSURF.blit(easy, easyRect)
    DISPLAYSURF.blit(med, medRect)
    DISPLAYSURF.blit(hard, hardRect)
    pygame.display.update()
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if easyRect.collidepoint(mouseX, mouseY):
                    return 10
                elif medRect.collidepoint(mouseX, mouseY):
                    return 25
